        ...
        for e in range(args.epochs):
            ...
            while True:
                try:
                    ...
                    act_idx_out, act_probs_one, rl_outputs, mask_arr, _, _, values, gauc, ndcg, summary1 = model.predict(x_data, y)
                    rewards = evaluator.predict(rl_outputs).reshape((-1, args.slate_size))
                    
                    

                    for _ in range(args.update_steps):
                        total_loss, mean_return, summary2, step = model.train(x_data, rl_outputs, act_probs_one, act_idx_out, rewards, mask_arr, c_entropy, values)
                    ...
                    if step % (100 * int(args.update_steps)) == 0:
                        ...
                        while True:
                            try:
                                ...
                                act_idx_out, act_probs_one, rl_outputs, mask_arr, _, _, _, gauc, ndcg, _ = model.predict(x_data_e, y_e)
                                rewards = evaluator.predict(rl_outputs).reshape((-1, args.slate_size))
